...
{
    public class NadamOptimizerFunction_M : IOptimizerMultipleLayerFunction
    {
        ...
        public void UpdateWeight(IMultipleLayer target, Array<FloatArray2D> dW)
        {
            for(int i = 0; i < target.ConectionsSize.Length; i++)
            {
                ...
                target.KnowlodgeMatrix[i] -= ( target.LearningRate / ( Adam_v_ws_hat.Sqrt() + e ) ) * ( b1 * Adam_m_ws_hat + ( ( ( 1 - b1 ) * dW[i] ) / ( 1 - b1 ) ) );
            }
        }
    }
}
...
namespace VI.Neural.ANNOperations
{
    public class ANNMultipleActivatorOperations : ISupervisedMultipleOperations
    {
        ...
        {        
            ...
            for(int i = 0; i < _target.ConectionsSize.Length; i++)  _target.SumVector[i] = (feed[i].T * _target.KnowlodgeMatrix[i]).SumLine();
        ...
        public virtual Array<FloatArray> BackWard(FloatArray backprop)
        {
            ...
            for(int i = 0; i < _target.ConectionsSize.Length; i++) back[i] = (_target.ErrorVector * _target.KnowlodgeMatrix[i]).SumColumn();
            return back;
        }
        ...
        public virtual void ComputeGradient(Array<FloatArray> inputs)
        {
            ...
            for(int i = 0; i < _target.ConectionsSize.Length; i++) _target.GradientMatrix[i] = inputs[i].T * _target.ErrorVector;
        }

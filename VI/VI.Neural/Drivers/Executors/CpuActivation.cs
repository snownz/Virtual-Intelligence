...
{
    public class CpuActivation : IActivationExecutor
    {
        public FloatArray ArcTANH(FloatArray v)
        {
            var result = new FloatArray(v.Length);
            for(int x = 0; x < v.Length; x++) result[x] = (float)Math.Pow( Math.Tanh( v[x] ), -1 );
            return result;
        }   
        public FloatArray DArcTANH(FloatArray v)
        {
            var result = new FloatArray(v.Length);
            for(int x = 0; x < v.Length; x++) result[x] = 1 / ( v[x] * v[x] + 1);
            return result;
        }
        ...
        {
            var result = new FloatArray(v.Length);
            for(int x = 0; x < v.Length; x++) result[x] =  Math.Max( .001f * v[x], v[x] );
            return result;
        }
        public FloatArray DLeakRelu(FloatArray v)
        {
            var result = new FloatArray(v.Length);
            for(int x = 0; x < v.Length; x++) result[x] = (v[x] >= .001f ? 1 : 0) + .001f;
            return result;
        }
        ...
        {
            var result = new FloatArray(v.Length);
            for(int x = 0; x < v.Length; x++) result[x] =  1f / ( 1f + (float) Math.Exp( -2f * v[x] ) );
            return result;
        }
        public FloatArray DSigmoid(FloatArray v)
        {
            var result = new FloatArray(v.Length);
            for(int x = 0; x < v.Length; x++) result[x] = 2f * v[x] * ( 1 - v[x] );
            return result;
        }
        ...
        {
            var result = new FloatArray(v.Length);
            for(int x = 0; x < v.Length; x++) result[x] = 1f- ( v[x] * v[x] );
            return result;
        }
    }
}